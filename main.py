...
while True:
    ...
    try:
        ...
        async def on_ready():
            ...
            logging.info(f'Bejelentkezve: "{client.user}"')
        ...
        async def on_message(message):
            ...
            # set
            if dc.cmd(msg, os.environ["KEY"], [["set", True]]):
                await gcset.main(content, os.environ["KEY"])

            # 8ball
            if dc.cmd(msg, prefix, [["8ball", True]]):
            ...
            # kép
            if dc.cmd(msg, prefix, [["kép", True], ["kep", True]]):
                await gckep.main(msg, prefix, client)

            # *************************************************************************************

            # kagi
            if dc.cmd(msg, prefix, [["kagi", True], ["kaga", True]]):
                await dc.send(msg, ":poop:")

            # bajsz
            if dc.cmd(msg, prefix, [["bajsz", True]]):
                bajszLista = [
                    ":disguised_face:",
                    ":man:",
                    ":bearded_person:",
                ...
                if len(msg.content) <= len(prefix) + len("gtn") + 1:
                    ...
                    try:
                        ...
                        if (
                            ...
                            await dc.send(
                                msg,
                                f"Van a billentyűteted. Írj be egy számot 1 és {msg.content[len(prefix) + 4:]} között. Nyomd meg az [ENTER] gombot. Visszakapod azt hogy `Kisebb!`, `Nagyobb!`, vagy azt hogy `Jippí!`. Most már érted?!\nIllusztráció: https://cdn.discordapp.com/attachments/741670562585247835/843095037079715930/unknown.png\n*A tájékoztatás nem teljeskörő. További infókért kérdezd meg anyádat.*",
                            )
                        else:
                            await dc.send(
                                msg,
                                "Bocs, de amit beírtál az nem okés. Adj meg egy EGÉSZ számot, ami NAGYOBB mint 1, de KISEBB mint 2.147.483.647!",
                            )
